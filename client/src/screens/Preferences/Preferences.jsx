...
const Preferences = () => {
  const {theme, setTheme, changeTheme, switchState} = useContext(ThemeContext);
  ...
  const {user, setUser, handleSignOut, handleDeleteAccount} =
    useContext(UserContext);

  const {allToDos, deleteAllToDos} = useContext(ToDoContext);
      ...
      <View
        style={
          height >= 500
            ...
            : styles.containerScreenLANDSCAPE
        }>
        ...
        {!toggleDeleteWarn ? (
          <>
            ...
            <View style={styles.toggleContainer}>
              <Text style={[{color: colors.text}, styles.test]}>
                Toggle Theme
              </Text>
              <Switch onChange={() => changeTheme()} value={switchState} />
            </View>
          </>
        ) : (
          <>
