...
const ToDo = props => {
  ...
  const {setHomeList} = useContext(ListsContext);
  ...
  const {title, _id, notes, deleteToDo, panRef, scrollRef, listName, editTask} =
    props;
  ...
  const panGestureHandler = Gesture.Pan()
    ...
    .onEnd(() => {
      const willDismiss = translateX.value < -windowWidth * 0.3;
      if (willDismiss) {
        translateX.value = withTiming(-windowWidth);
        itemHeight.value = withTiming(0);
        marginVertical.value = withTiming(0, undefined, isFinished => {
  ...
  return (
    <Animated.View style={[styles.container, rAnimatedContainerStyle]}>
      ...
      <GestureDetector
        ...
        <Animated.View
          style={[rAnimatedSwipe, {backgroundColor: colors.card}, styles.card]}>
          {detailsVisible && (
            <ToDoDetails
              title={title}
              _id={_id}
          ...
          <GestureDetector gesture={DoubleTapGestureHandler}>
            <Text
              style={[
                {color: colors.text},
                styles.text,
                orientation === 'LANDSCAPE' ? {fontSize: 20} : {fontSize: 25},
              ]}
              onPress={() => fireHaptic()}
              numberOfLines={1}>
              {title}
